...
class TrafficControllerNode(Node):
    ...
    def __init__(self):
        ...
        self.emergency_timeout_sec = 20.0
    ...
    def emergency_callback(self, msg):
        try:
            ...
            self.get_logger().error(f"Invalid emergency JSON: {e}")
            return

        self.emergency_active = data.get("emergency_present", False)

        self.current_emergency_type = data.get("type", EmergencyType.NONE)
        self.current_emergency_vehicle_id = data.get("id", None)
        self.current_emergency_position = data.get("pos", 0.0)
        self.current_emergency_speed = data.get("speed", 0.0)
        self.current_emergency_siren = data.get("siren", False)
        ...
        if self.emergency_active:
            self.get_logger().warn(
                ...
                f"pos={self.current_emergency_position:.2f} | "
                f"speed={self.current_emergency_speed:.2f} | "
                f"siren={self.current_emergency_siren}"
            )
        else:
    ...
    def update_emergency_state(self):
        ...
        if elapsed_time > self.emergency_timeout_sec:
            ...
            self.current_emergency_type = EmergencyType.NONE
            self.current_emergency_vehicle_id = None
            self.current_emergency_position = 0.0
            self.current_emergency_speed = 0.0
            self.current_emergency_siren = False
